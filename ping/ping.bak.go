...
func main() {
	flag.Parse()
	if os.Getuid() != 0 {
		log.Fatalln("test disabled; must be root")
	}
	...
	if *srchost != "" {
		laddr, err = net.ResolveIPAddr("ip4", *srchost)
		if err != nil {
			log.Fatalf(`net.ResolveIPAddr("ip4", %v") = %v, %v`, *srchost, laddr, err)
		}
	}

	raddr, err := net.ResolveIPAddr("ip4", *dsthost)
	if err != nil {
		log.Fatalf(`net.ResolveIPAddr("ip4", %v") = %v, %v`, *dsthost, raddr, err)
	}

	c, err := net.ListenIP("ip4:icmp", laddr)
	if err != nil {
		log.Fatalf(`net.ListenIP("ip4:icmp", %v) = %v, %v`, *srchost, c, err)
	}
